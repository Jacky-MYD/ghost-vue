<!-- <template>
    <table class="g-table-body">
        <colgroup>
            <col :name="column.id" v-for="(column) in columns" :key="column.id" :width="column.realWidth">
        </colgroup>
        <tbody>
            <tr class="g-table__row" v-for="(item,index) in getData(data)" :key="index">
                <td></td>
            </tr>
        </tbody>
    </table>
    ...
    export default {
        ...
        render(h) {
            ...
            <table
                ...
                <colgroup>
                {
                    this.columns.map(column => <col name={ column.id } key={column.id} width={column.realWidth} />)
                }
                </colgroup>
                <tbody>
                    {
                    ...
                    <g-tooltip effect={ this.table.tooltipEffect } placement="top" ref="tooltip" content={ this.tooltipContent }></g-tooltip>
                </tbody>
            </table>
            )
        },
        methods: {
            ...
            isColumnHidden(index) {
                if (this.fixed === true || this.fixef === 'left') {
            ...
            getRowClass(row, rowIndex) {
                const classes = ['g-table__row']
                if (this.table.highlightCurrentRow && row === this.store.states.currentRow) {
                    classes.push('current-row')
                }

                if (this.stripe && rowIndex % 2 === 1) {
                    classes.push('g-table__row--striped')
                }
            ...
            rowRender(row, $index, treeRowData) {
                const { treeIndex, columns, firstDefaultColumnIndex } = this
                const columnsHidden = columns.map((column, index) => this.isColumnHidden(index))
                const rowClasses = this.getRowClass(row, $index)
                let display = true
                if (treeRowData) {
                    rowClasses.push('g-table__row--levg-' + treeRowData.level)
                ...
                if (this.hasExpandColumn && isRowExpanded(row)) {
                    ...
                    return [[
                        ...
                        <tr key={'expanded-row__' + tr.key}>
                            <td colspan={ this.columnsCount } class="g-table__expanded-cell">
            ...
            handleMouseEnter: debounce(30, function(index) {
                this.store.commit('SET_HOVER_ROW', index)
            }),
            ...
            handleCellMouseEnter(event, row) {
                ...
                if (!(hasClass(cellChild, 'g-tooltip') && cellChild.childNodes.length)) {
                    return
                }
        ...
        watch: {
            'store.states.hoverRow'(newVal, oldVal) {
                ...
                raf(() => {
                    const rows = this.$el.querySelectorAll('.g-table__row')
