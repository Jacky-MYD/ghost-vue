...
// 处理column
export const dealColumns = (columns) => {
    const result = []
    columns.forEach(column => {
        if (column.children) {
            result.push.apply(result, dealColumns(column.children))
        } else {
            result.push(column)
        }
    })
...
export const getColumnByCell = (table, cell) => {
    const matches = (cell.className || '').match(/g-table_[^\s]+/gm);
    if (matches) {
...
export const orderBy = (array, sortKey, reverse, sortMethod, sortBy) => {
...
export const mergeOptions = (defaults, config) => {
    const options = {}
...
// compose函数,返回函数集 functions 组合后的复合函数
